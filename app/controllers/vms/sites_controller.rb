class Vms::SitesController < ApplicationController
  ...
  def show
    ...
    staff = (@site.staff.map {|s| s.user[:source] = 'manual'; s.user[:staff_id] = s.id; s.user } + @site.teams.map{ |t| t.audience.users }.flatten.map{|ui| ui[:source] = 'team'; ui}).flatten.uniq
